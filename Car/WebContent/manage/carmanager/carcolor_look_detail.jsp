    	...
    	$(function(){
    		...
    		$(".back").each(function(){
    			$(this).click(function(){
					...
					var colorName = $(this).find("span").eq(0).html();
					var colorRemark = $(this).find("span").eq(1).html();
					//将相应的信息显示到相应的输入框
					$("#spanName").html(colorName);
					$("#carColorRemark").val(colorId);
					$("#carColorId").val(colorRemark);
					$("#carColorName").val(colorName);
					//将详情框显示
    		...
    		$("#removeNone").click(function(){
				//确保输入框是只读的
				$("#carColorName").attr("readonly","readonly");
				$("#carColorRemark").attr("readonly","readonly");
				//隐藏详情框
    		...
    		$("#btnUpdate").click(function(){
    			...
    			if(method == "修改"){
    				//将输入框输入启用
    				$("#carColorName").removeAttr("readonly","false");
    				$("#carColorRemark").removeAttr("readonly","false");
    				//将按钮改为保存和取消按钮
    				$("#btnUpdate").html("保存");
    				$("#btnDelete").html("取消");
    			}else if(method == "保存"){
...
<body class="bg-1">
	<div class="pageheader" id="pageheader">
	  ...
	  <!-- 管理界面标题与查询框start -->
      <h2><i class="fa fa-tachometer"></i> 汽车颜色管理
       <span>
        <form action="${pageContext.request.contextPath}/CarColorServlet">
         <div class="search" id="main-search">
         	<input type="hidden" name="method" value="listByName"/>
            <input type="text" placeholder="请输入颜色名称" id="txt-search" name="txt-search" value="${keyWord}">
	  ...
	  <a href="${pageContext.request.contextPath}/manage/carmanager/carcolor_add.jsp" class="btn btn-hotpink margin-bottom-20" id="btnAdd">
         <span>添加</span>
      </a>
	...
	<!-- 卡片列表start -->
	<div class="row cards">
<c:forEach items="${pageBean.datas}" var="carColor">

	

	<c:choose>
	  <c:when test="${carColor.car_Colorid%4==1}">
        <div class="card-container col-lg-3 col-sm-6 col-sm-12">
          <div class="card card-redbrown hover">
            <div class="front"> 
              ...
              <div class="media">        
                ...
                <div class="media-body">
                  <small>颜色编号：${carColor.car_Colorid }</small>
                  <h2 class="media-heading animate-number" data-value="${carColor.car_Colorname }" data-animation-duration="1500">0</h2>
                </div>
              </div> 
            ...
            <div class="back" name="${carColor.car_Remark }">
              <span style="display:none;">${carColor.car_Colorname }</span>
              <span style="display:none;">${carColor.car_Colorid }</span>
              <a href="#">
	  ...
	  <c:when test="${carColor.car_Colorid%4==2}">
        <div class="card-container col-lg-3 col-sm-6 col-sm-12">
	  ...
	  <c:when test="${carColor.car_Colorid%4==3}">
        <div class="card-container col-lg-3 col-sm-6 col-sm-12">
      ...
      <!-- 分页start -->
       <div id="TableTail" align="center">
		...
		<a href="${pageContext.request.contextPath }/CarColorServlet?method=listByName&txt-search=${keyWord}">首页</a>
		

		<a href="${pageContext.request.contextPath }/CarColorServlet?method=listByName&page=${pageBean.currentPage-1}&txt-search=${keyWord}">上一页 </a>
...
<c:if test="${pageBean.currentPage!= pageBean.pageNum}">
		<a href="${pageContext.request.contextPath }/CarColorServlet?method=listByName&page=${pageBean.currentPage+1}&txt-search=${keyWord}">下一页 </a>
</c:if>			
		<a href="${pageContext.request.contextPath }/CarColorServlet?method=listByName&page=${pageBean.pageNum}&txt-search=${keyWord}">末页</a>
    </div> 
    <!-- 分页end -->
	  ...
	  <div class="col-sd-4" id="detail">
       <section class="tile color transparent-white">
        ...
        <div class="tile-body" id="detailBody">
          ...
          <form class="form-horizontal" role="form" action="${pageContext.request.contextPath}/CarColorServlet">
            
            <input type="hidden" name="method" value="del" id="hiddenInput"/>
            
            <div class="form-group">
              <label for="carTypeId" class="col-sm-4 control-label">颜色编号：</label>
              <div class="col-sm-8">
                <input type="text" class="form-control" id="carColorId" name="carColorId" readonly="readonly">
              </div>
            </div>

            <div class="form-group">
              <label for="carTypeName" class="col-sm-4 control-label">颜色名称：</label>
              <div class="col-sm-8">
                <input type="text" class="form-control" id="carColorName" name="carColorName" readonly="readonly">
              </div>
            </div>

            <div class="form-group">
              <label for="carTypeRemark" class="col-sm-4 control-label">颜色备注：</label>
              <div class="col-sm-8">
                <input type="text" class="form-control" id="carColorRemark" name="carColorRemark" readonly="readonly">
              </div>
            </div>
